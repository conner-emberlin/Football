...
namespace Football.Client.Services
{
    public class PlayersService(IRequests requests) : IPlayersService
    {
        ...
        public async Task<bool> PostInjuryConcernRequest(InjuryConcernsModel concern) => await requests.Post<bool, InjuryConcernsModel>("/player/injury-concern", concern);
        public async Task<List<InjuryConcernsModel>?> GetInjuryConcernsRequest() => await requests.Get<List<InjuryConcernsModel>?>("/player/injury-concerns");
        public async Task<bool> DeleteInjuryConcernRequest(int playerId, bool isSuspension = false) 
        {
            var path = string.Format("/player/injury-concern/{0}", playerId);
            if (isSuspension) path += "?isSuspension=true";
            return await requests.Delete<bool>(path);
        } 

    ...
    public class ApiModelAutomapperProfile : Profile
    {
        public ApiModelAutomapperProfile()
        {
            ...
            CreateMap<QualityStarts, QualityStartsModel>()
                .ForMember(qm => qm.PoorStartPercentage, o => o.MapFrom(q => q.GamesPlayed > 0 ? Math.Round(((double)q.PoorStarts / (double)q.GamesPlayed) * 100, 2) : 0))
                .ForMember(qm => qm.GoodStartPercentage, o => o.MapFrom(q => q.GamesPlayed > 0 ? Math.Round(((double)q.GoodStarts / (double)q.GamesPlayed) * 100, 2) : 0))
                .ForMember(qm => qm.GreatStartPercentage, o => o.MapFrom(q => q.GamesPlayed > 0 ? Math.Round(((double)q.GreatStarts / (double)q.GamesPlayed) * 100, 2) : 0))
                .ForMember(qm => qm.QualityCount, o => o.MapFrom(q => q.GoodStarts + q.GreatStarts))
                .ForMember(qm => qm.QualityPercentage, o => o.MapFrom(q => q.GamesPlayed > 0 ? Math.Round(((double)(q.GoodStarts + q.GreatStarts) / (double)q.GamesPlayed) * 100, 2) : 0));
        }
